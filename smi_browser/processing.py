...
def build_proc_params(
    ...
    *,
    ...
    # Derived analysis products.  ``virtual_axes`` and ``peak_fits`` operate
    # on ``result.per_frame_iq`` from ``reduce_smi_combined``; GI only accepts
    # ``line_cuts`` for qxy/qz frame stacks.
